...
export default class FormationTrack extends SvgTrack {
  ...
  plot() {
    ...
    if (!data || data.length === 0) {
      ...
      const newFormations = selection.enter().append('g')
        .classed('formation', true)
        .attr('transform', d => `translate(0,${d.yFrom})`);

      const box = newFormations.append('rect');
      setAttrs(box, d => {
        let from = d.yFrom;
        let to = d.yTo;
      ...
      const l1 = newFormations.append('line');
      setAttrs(l1, {
        x1: xscale(0.25),
        x2: xscale(0.75),
        y1: 0,
      ...
      const l2 = newFormations.append('line');
      setAttrs(l2, d => ({
        x1: xscale(0.5),
        x2: xscale(0.5),
        y1: 0,
      ...
      const l3 = newFormations.append('line');
      setAttrs(l3, d => ({
        x1: xscale(0.25),
        x2: xscale(0.75),
        y1: d.yTo - d.yFrom,
