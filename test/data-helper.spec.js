...
describe('DataHelper', () => {
  ...
  it('should filter data points based on provided domain and an excess factor', () => {
    expect(DataHelper.filterData(datapoints, [3, 5], 0)).toEqual([
      [3, 20], [4, 10], [5, null],
    ]);

    expect(DataHelper.filterData(datapoints, [3, 5], 0.5)).toEqual([
      [2, 10], [3, 20], [4, 10], [5, null], [6, null],
    ]);

    expect(DataHelper.filterData(datapoints, [4, 5], 1)).toEqual([
      [3, 20], [4, 10], [5, null], [6, null],
    ]);

    // should not filter points required to draw the first and last point
    // that are within the domain
    expect(DataHelper.filterData(datapoints, [4.1, 4.9], 0)).toEqual([
      [4, 10], [5, null],
    ]);
  });

  it('should be able to resample data points to a given ratio', () => {
    const data = range(0, 10000, 1).map(d => [d, d]);
    expect(DataHelper.resample(data, 1)).toEqual(data);
    expect(DataHelper.resample(data, 1000)).toEqual([
      [0, 0],
      [500.5, 500.5],
      [1500.5, 1500.5],
      [2500.5, 2500.5],
    ...
    expect(DataHelper.resample(data, 200)).toEqual([
      [0, 0],
      [100.5, 100.5],
      [300.5, 300.5],
      [500.5, 500.5],
  ...
  it('should be able to trim undefined data (no more than one neighbouring null value)', () => {
    expect(DataHelper.trimUndefinedValues(datapoints)).toEqual([
      [0, null],
      [2, 10],
      [3, 20],
      [4, 10],
  ...
  it('should be able to merge two dataseries into one correlated serie', () => {
    ...
    const serie2 = serie1.slice(3, 10).map(d => [d[0], -d[1]]);
